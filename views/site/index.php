<div id="content" class="wrapper">	
	<div class="column content-up">
		<div class="img">
				
		</div>	
		<div class="nav">
			<div class="btn">
				<div class="btn-top">
				</div>
				<div class="btn-down">
					<a href=""><p>最新</p></a>
				</div>
			</div>
			<div class="smart">
				<a href=""><p>智能</p></a>
			</div>
			<div class="list">
				<ul class="list-ul">
					<li><a href=""><p>小米</p></a></li>
					<li><a href=""><p>Tesla</p></a></li>
					<li><a href=""><p>Oculus</p></a></li>
					<li><a href=""><p>Uber</p></a></li>
					<li><a href=""><p>比特币</p></a></li>
					<li><a href=""><p>更多</p></a></li>
				</ul>
			</div>
		</div>
	</div>
	<div class="column content-down">
	    <div class="col-11 left">
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="#" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag3.png') no-repeat;width:39px;height:20px"><p class="fs-10">网站</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26 </p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	
	    	</div>
	    	<div class="login bg-login-green">
	    		<div class="text pt-12 pb-10 ml-60"><p class="fs-13 lp-1 sw">登陆账号，我们将为您提供更多这类的文章</p></div>	
	    		<div class="login-btn">
	    			<div class="lgbtn fl ml-60"><p class="fs-14">微博登陆</p></div>	
		    		<div class="text_or fl ml-14 mr-14"><p class="fs-14">或</p></div>
		    		<div class="lgbtn fl"><p class="fs-14">QQ登陆</p></div>	
	    		</div>
	    		
	    	</div>
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag2.png') no-repeat;width:39px;height:20px"><p class="fs-10">应用</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26</p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	</div>
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag4.png') no-repeat;width:39px;height:20px"><p class="fs-10">硬件</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26</p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	</div>
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag1.png') no-repeat;width:39px;height:20px"><p class="fs-10">网站</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26</p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	</div>
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag5.png') no-repeat;width:39px;height:20px"><p class="fs-10">网站</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26</p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	</div>
	    	<div class="post divider">
	    		<div class="img-holder">
	    			<img src="" width="260" height="180"/>
	    			<div class="icon-type">
	    				<div style="background:url('/img/icon/tag5.png') no-repeat;width:39px;height:20px"><p class="fs-10">网站</p></div>
	    			</div>
	    		</div>	
	    		<div class="text">
	    			<p class="fs-21 lp-3">韩国电子优惠券服务Spoqa融资200万美元，并进入日本市场</p>	
	    			<p class="fs-12 lp-1">作者<strong>Betty</strong> - 2014/08/26</p>
	    			<p class="fs-14 lp-3">总部是在韩国首尔的创业公司Spoqa做的一种积分服务Dodo。<a class="" href="#"><strong>阅读全文</strong></a></p>
	    			
	    		</div>
	    	</div>
	    	<div class="load loadgray">
				<p class="fs-14 fl loadnext">正在为您加载第X页面</p>	
				<a class="fs-14 fr" href="#">下一页</a>
			</div>
	    </div>

	   <!--中右内容--> 
	    <div class="col-4 right">
	    	<div class="hot-list bg-click">
	    		<div class="hot">
	    			<div class="img-line-up">
	    				
	    			</div>	
	    				<a href="#"><img src="#" alt=""/></a>	
	    			<p class="fs-14">PayPal联合创始人Peter Thie焦点哲学</p>
	    		</div>
	    		<div class="hot">
	    			<div class="img-line-down">
	    				
	    			</div>	
	    			<div>
	    				<a href="#"><img src="#" alt=""/></a>	
	    			</div>
	    			<p class="fs-14">PayPal联合创始人Peter Thie焦点哲学</p>
	    			
	    		</div>
				<!-- 屏幕header-->
	    		<div class="header">
	    			<p class="fs-13">优质评论</p>
	    		</div>
	    		<HR align=center width=86.66666667% color=#ee6350 SIZE=2 style="margin-left:20px;" noShade>

	    		<div class="article divider">
	    			<div class="customer">
	    				<p class="fs-14 orange fl">用户名</p>
	    				<p class="fs-14 fl"> · 一小时前 </p>
	    			</div>	
	    			<div class="cont">
	    				<p class="fs-14 lp-2">看了那么多有关于创业的例子，成功的，失败的都有，只想说：成功的路上没有太多可鉴和复制的。别人走过的路，你可以走，但走的方式和方法要更出色。没人走过的路，你也可以走，但走的很坎坷很纠结。</p>
	    			</div>
	    			<div class="from fs-15 lp-1">
	    				<p class="fs-14 lp-1">评论于&nbsp;&nbsp;<a class="fs-14 lp-1" href="#">周鸿祎的互联网方法论</a></p>
	    			</div>
	    		</div>
	    		<div class="article divider">
	    			<div class="customer">
	    				<p class="fs-14 orange fl">用户名</p>
	    				<p class="fs-14 fl"> · 一小时前 </p>
	    			</div>	
	    			<div class="cont">
	    				<p class="fs-14 lp-2">看了那么多有关于创业的例子，成功的，失败的都有，只想说：成功的路上没有太多可鉴和复制的。别人走过的路，你可以走，但走的方式和方法要更出色。没人走过的路，你也可以走，但走的很坎坷很纠结。</p>
	    			</div>
	    			<div class="from fs-15 lp-1">
	    				<p class="fs-14 lp-1">评论于&nbsp;&nbsp;<a class="fs-14 lp-1" href="#">周鸿祎的互联网方法论</a></p>
	    			</div>
	    		</div>
	    		<div class="article divider">
	    			<div class="customer">
	    				<p class="fs-14 orange fl">用户名</p>
	    				<p class="fs-14 fl"> · 一小时前 </p>
	    			</div>	
	    			<div class="cont">
	    				<p class="fs-14 lp-2">看了那么多有关于创业的例子，成功的，失败的都有，只想说：成功的路上没有太多可鉴和复制的。别人走过的路，你可以走，但走的方式和方法要更出色。没人走过的路，你也可以走，但走的很坎坷很纠结。</p>
	    			</div>
	    			<div class="from fs-15 lp-1">
	    				<p class="fs-14 lp-1">评论于&nbsp;&nbsp;<a class="fs-14 lp-1" href="#">周鸿祎的互联网方法论</a></p>
	    			</div>
	    		</div>
				
				<!-- 二维码-->
				<div class="qrcode">
					<p class="text fs-12 lp-2">微信公众平台：搜索“创新设计”或扫描一下二维码:</p>
					<img src="#" alt=""/>
				</div>

	    	</div>
	    </div>
	</div>
	
</div>
