 ...
 *      </div>
 */
class postPrevNextPager extends LinkPager {
    ...
    public function renderPageButtons() 
    {
        ...
        $html .= '<div class="fr next '. ($currentPage == $pageCount - 1 ? 'disabled' : '') .'"><a class="post-next" href="'. ($currentPage < $pageCount - 1 ? $this->pagination->createUrl($currentPage + 1) : $this->pagination->createUrl($pageCount - 1)) .'">下一页</a></div>';
        $html .= '<div class="fl previous '. ($currentPage == 0 ? 'disabled' : '') .'"><a class="post-prev" href="'. ($currentPage > 0 ? $this->pagination->createUrl($currentPage - 1) : $this->pagination->createUrl(0)) .'"">上一页</a></div>';
